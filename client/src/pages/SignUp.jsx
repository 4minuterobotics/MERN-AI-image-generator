...
const SignUp = (props) => {
	...
	const submitHandler = async (e) => {
		...
		try {
			...
			const response = await fetch(`${props.backendDomain}/api/v1/user/signup`, {
				method: 'POST',
				headers: {
			...
			<PageAnimation>
				<Container className='max-w-2xl'>
					...
					<Form onSubmit={submitHandler}>
						...
						<div className='mb-3'>
							<Button className='bg-[#6469ff]' variant='primary' type='submit'>
								Sign Up
							</Button>
						</div>
						<div className='mb-3'>
							Already have an account?{' '}
							<Link to={`/signin?redirect=${redirect}`} className=' font-bold text-blue-600'>
								Sign-In
							</Link>
						</div>
					</Form>
